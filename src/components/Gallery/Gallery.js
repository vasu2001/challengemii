    ...
    return (
        <div className={`gallery ${display===-1 ?'close':null}`}>   
            ...
            <div className='gallery-action-box'>
                <a className='btn-gallery-vote' onClick={()=>onLike(i)}>{voted?'Un-vote':'Vote'}</a>
                <a className='btn-gallery-share' onClick={onShare}>Share</a>
            </div>
            <a className={`btn-left ${i===0?'close':null}`} onClick={onPrev} ><BsChevronLeft /></a>
            <a className={`btn-right ${i===n-1?'close':null}`} onClick={onNext}><BsChevronRight /></a>
        </div>
    )
}
